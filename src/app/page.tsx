...
import { Terminal, Briefcase, Mail, MessageSquare } from 'lucide-react';
    ...
    <main className="selection:bg-accent selection:text-white min-h-screen flex flex-col justify-center py-12 px-4 md:px-6 overflow-x-hidden relative">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
        ...
        >
          <div className="flex flex-col md:flex-row gap-6 md:items-end">
            ...
            <div className="retro-border bg-accent p-6 md:p-8 retro-shadow inline-block flex-1">
              <h1 className="text-5xl sm:text-7xl md:text-8xl font-black text-white tracking-tighter leading-[0.85] uppercase italic">
                Dionatha <br /> Goulart
              </h1>
            </div>
          </div>
          ...
          <div className="retro-border bg-card p-6 md:p-8 retro-shadow-sm space-y-4 relative overflow-hidden">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-black italic text-accent flex items-center gap-2">
              ...
              <TypingText text="Sistemas de Ponta a Ponta." speed={70} />
            </h2>
        ...
        >
          <Link href="/dev" className="group">
            <div className="retro-border bg-card p-1 retro-shadow-sm group-hover:retro-shadow transition-all transform group-hover:-translate-y-2 relative overflow-hidden group-hover:bg-[var(--hub-dev-hover)] group-hover:text-white">
              {/* Terminal Title Bar */}
              <div className="bg-accent text-white px-3 py-1.5 flex justify-between items-center mb-1 group-hover:bg-white group-hover:text-[var(--hub-dev-hover)] transition-colors">
                <span className="font-mono text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <Logo className="w-3 h-3 text-white group-hover:text-[var(--hub-dev-hover)]" />
                  SOFTWARE_ENG.EXE
                </span>
                <div className="flex gap-1">
                  <div className="w-1.5 h-1.5 rounded-full border border-white group-hover:border-[var(--hub-dev-hover)]" />
                  <div className="w-1.5 h-1.5 rounded-full bg-white group-hover:bg-[var(--hub-dev-hover)]" />
                </div>
              </div>

              <div className="p-5 sm:p-7 space-y-4 font-mono group-hover:bg-white/5 transition-colors">
                <div className="flex items-start gap-3">
                  <span className="text-accent font-bold group-hover:text-white">$</span>
                  <div>
                ...
                <div className="pt-4 border-t border-accent/10 group-hover:border-white/20 flex justify-between items-center opacity-40 group-hover:opacity-100">
                  <span className="text-[9px] uppercase tracking-widest bg-accent/10 group-hover:bg-white/20 px-2 py-0.5 rounded text-accent group-hover:text-white">Status: Stable</span>
                  <span className="text-xs font-black">RUN {">>"}</span>
                </div>
              </div>
            </div>
          </Link>
          ...
          <Link href="/ti" className="group">
            <div className="retro-border bg-card p-1 retro-shadow-sm group-hover:retro-shadow transition-all transform group-hover:-translate-y-2 relative overflow-hidden group-hover:bg-[var(--hub-ti-hover)] group-hover:text-white">
              {/* Terminal Title Bar */}
              <div className="bg-accent text-white px-3 py-1.5 flex justify-between items-center mb-1 group-hover:bg-white group-hover:text-[var(--hub-ti-hover)] transition-colors">
                <span className="font-mono text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <Logo className="w-3 h-3 text-white group-hover:text-[var(--hub-ti-hover)]" />
                  IT_OPERATIONS.SH
                </span>
                <div className="flex gap-1">
                  <div className="w-1.5 h-1.5 rounded-full border border-white group-hover:border-[var(--hub-ti-hover)]" />
                  <div className="w-1.5 h-1.5 rounded-full bg-white group-hover:bg-[var(--hub-ti-hover)]" />
                </div>
              </div>
              ...
              <div className="p-5 sm:p-7 space-y-4 font-mono group-hover:bg-white/5 transition-colors">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm font-bold opacity-60 uppercase leading-tight group-hover:opacity-100">
                      Infra • Redes • Linux • Auto
                    </p>
                  </div>
                </div>

                <div className="pt-4 border-t border-accent/10 group-hover:border-white/20 flex justify-between items-center opacity-40 group-hover:opacity-100">
                  <span className="text-[9px] uppercase tracking-widest bg-accent/10 group-hover:bg-white/20 px-2 py-0.5 rounded text-accent group-hover:text-white">Active_Session</span>
                  <span className="text-xs font-black">SSH {">>"}</span>
                </div>
              </div>
            </div>
          </Link>
      ...
      <div className="max-w-7xl mx-auto mt-16 md:mt-24 w-full flex flex-col md:flex-row justify-between items-center gap-8 border-t-2 border-border-custom/10 pt-12 relative z-10">
        <div className="flex flex-wrap gap-4 sm:gap-6 justify-center">
          {[
            { name: 'LinkedIn', url: 'https://linkedin.com/in/dionathagoulart', icon: Briefcase },
            { name: 'GitHub', url: 'https://github.com/DionathaGoulart', icon: Terminal },
            { name: 'Email', url: 'mailto:dionatha.work@gmail.com', icon: Mail },
            { name: 'WhatsApp', url: 'https://wa.me/5551998544525', icon: MessageSquare }
          ].map((social) => (
            <Link 
              key={social.name}
              href={social.url}
              className="retro-border bg-card p-3 sm:p-4 hover:bg-accent hover:text-white transition-all relative group retro-shadow-sm hover:retro-shadow-sm hover:-translate-y-1 active:translate-y-0"
              title={social.name}
            >
        ...
        <div className="font-mono text-[10px] opacity-30 uppercase tracking-[0.2em] text-center md:text-right">
          DG_OS_CORE // BUILD_2026.06.17<br />
          PORTIFOLIO_ROOT_HUB
        </div>
      </div>
    </main>
