  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {submitStatus === 'error' && (
          <div
            className="p-4 bg-theme-error/10 border border-theme-error rounded-lg"
            role="alert"
            aria-live="assertive"
          >
            <P color="error">
              Ocorreu um erro ao enviar a mensagem. Tente novamente ou entre em
        ...
        <Button
          ...
          className="w-full"
          aria-describedby="botão-de-enviar"
        >
          {isLoading ? 'Enviando...' : 'Enviar Mensagem'}
        </Button>
        <div id="botão-de-enviar" className="sr-only">
          Botão para enviar o formulário de contato
        </div>
      </form>
    </div>
  )
...
const ContactInfo: React.FC = () => {
  ...
  const directContactCards: ContactCard[] = useMemo(
    () => [
      {
        icon: <EmailIcon />,
        title: 'Email',
  ...
  const socialCards: ContactCard[] = useMemo(
    () => [
      {
        icon: <GitHubIcon />,
        title: 'GitHub',
        subtitle: '/dionathagoulart',
  ...
  return (
    <div className="space-y-6">
      ...
      >
        Prefere outro meio de contato? Aqui estão todas as formas de entrar em
        contato comigo.
      </P>
      ...
      <div className="space-y-4 mb-12">
        {directContactCards.map((card, index) => (
          <Card
            ...
            aria-label={`Entrar em contato via ${card.title}: ${card.subtitle}`}
          />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {socialCards.map((card, index) => (
          <Card
            ...
            subtitle={card.subtitle}
            onClick={card.onClick}
            aria-label={`Visitar perfil no ${card.title}: ${card.subtitle}`}
          />
        ))}
      </div>
    </div>
